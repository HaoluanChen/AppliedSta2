...
```{r}
dm %>% group_by(Age) %>% summarize(across(Female:Total, sd))
```
...
```{r}
q4data <- read_table("https://www.prdh.umontreal.ca/BDLC/data/ont/Population.txt", skip = 2, col_types = "dcddd")
head(q4data)
```

```{r}
q4data <- q4data %>% left_join(dm, q4data, by = c("Year", "Age")) %>% 
  group_by(Year) %>% mutate(TotalPopF = sum(Female.x), TotalPopM = sum(Male.x), 
                                                                                                WeightedAvgF = (Female.x * Female.y)/TotalPopF, 
                                                                                                WeightedAvgM =(Male.x * Male.y)/TotalPopM)
q4data
q4data <- q4data %>% group_by(Year) %>% 
  summarize(across(WeightedAvgF:WeightedAvgM,sum, na.rm = TRUE))
q4data
```

```{r fig.width=6}
q4data %>% rename(Female = WeightedAvgF, Male = WeightedAvgM) %>%
  pivot_longer(Female:Male, names_to = "Sex", values_to = "mortality") %>% 
  filter(Year != 2020) %>%
  ggplot(aes(x = Year, y = mortality, color = Sex)) + 
  geom_line() +
  theme_bw(base_size = 12) +
  labs(title = "Weighted Average Mortality rates over time, Ontario",
       y = "Mortality rate")
```

From the line plot above, we see a decreasing trend from 1921 to late 90's for the mortality rate for both male and female. Then it started fluctuate around 0.007. Additionally, Male's mortality rate is almost always higher than female.
